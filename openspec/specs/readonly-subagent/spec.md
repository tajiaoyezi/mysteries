...
child MUST 把调用时`ToolContext.cwd` canonicalize为workspace root，并把它作为四个读取工具的强制边界。四个工具 MUST 在同一blocking worker/permit内先对原始path做pure lexical containment：普通relative path以canonical root为基准归一化，越界absolute/`..`及Windows UNC、device namespace、rooted-without-prefix、drive-relative、8.3/non-canonical alias或其他ambiguous form必须在任何target canonicalize前fail-closed；仅drive与normalized components均能和canonical root做lexical match（Windows ASCII casing可不同）的local-disk absolute path受支持，并 MUST 以canonical root加归一化relative suffix重建candidate，不得把用户提供的原始spelling交给filesystem。随后 MUST 从canonical root开始对candidate每个既有component执行no-follow `symlink_metadata`，任意symlink、junction或其他reparse point均在target canonicalize前fail-closed；最后canonicalize重建candidate、再次验证等于root或为其descendant，并只使用该canonical target执行实际I/O。不得在async runtime线程做blocking path解析，也不得验证后重新解析原始输入。目录walker加载的parent、`.ignore`与`.gitignore`规则文件也受同一边界约束：read root外ancestor规则不得读取或影响结果，read root内规则文件的symlink/reparse point必须在canonicalize和解析前fail-closed，普通规则文件canonical后仍须验证在read root内并从canonical path解析；preflight MUST 按既有ignore/hidden pruning逐层验证规则文件，被剪枝目录自身可能加载的control file仍验证，但不得继续探测actual walker不可达的descendant。read root内parent/nested规则的precedence与whitelist语义 MUST 保持既有行为，但parent规则命中调用者显式target或其ancestor时不得在到达该target前将其剪枝。target等于root只表示通过containment，不改变底层工具的输入类型：三个目录工具可在root目录执行，`read_file`成功case使用root下文件，而`read_file(root目录)`继续返回既有目录读取错误。在单次调用期间filesystem namespace未被并发修改的威胁模型下，所有上述拒绝 MUST 返回is_error，目标内容不得进入child history或后续Provider请求。普通root Agent未携带child read root时 MUST 保持既有绝对路径与gitignore行为。
...
#### Scenario: symlink、junction或reparse point在canonicalize前被拒绝
- **WHEN** child显式target的任一component为symlink、Windows junction或其他reparse point，无论其解析目标是否仍在workspace内
- **THEN** no-follow preflight在target canonicalize与实际I/O前拒绝调用；平台无法创建对应链接时测试必须明确记录skip原因

...
## Purpose
agent-execution-scope 定义一次 Agent run 可安全派生的运行边界：稳定 identity、parent→child 单向 cancellation、只能保持或收紧的 iteration/deadline/depth 预算，以及工具名与权限级 capability 子集。它是后续 child Agent / subagent 的内核安全基础，但本能力本身不提供调度器、child session、token 总预算或新用户命令。

...
- **THEN** 本轮以 `AgentEvent::Interrupted` 收场、状态回 `Idle`,provider 不被再次调用,agent task 继续存活;中断路径只发 `Interrupted`(不再紧跟冗余 `StatusChanged(Idle)`)
...
- **THEN** 依次:回送 `Deny`(pending 优先)/ 清选区(消费)/ 投 `Interrupt` + 记 last_cancel_at(第 1 次取消,推进下一条)/ `clear_queue()` + 投 `Interrupt`(快速连按清空)/ 投 `Interrupt`(无排队中断)/ `should_exit`(退出);优先级 pending > 选区 > 有排队两级取消 > 运行中中断 > 就绪退出

...
## Purpose
permission-gate 是工具执行前的集中放行决策点:`ReadOnly` 直接放行；有效 `Network`、`Edit`、`Execute` 经可注入的 `PermissionDecider` 取得 `Allow` / `Deny`，而不可授权 Network 在所有 mode 下 fail-closed。设计立场是门与 UI 解耦——decider 为 async 注入 seam，只消费 tool-owned Network preview；运行时 `PermissionMode`(`Normal` / `AcceptEdits` / `Yolo` / `Plan`)以纯函数 `auto_allows` 表达自动放行矩阵，但 gate 在 decider 返回后仍对 Network authorizability 做最终 clamp。拒绝不静默跳过，而是以区分 `UserDenied` / `NetworkUnauthorizable` 的 is_error `ToolResult` 入 history；Network 仅允许本次调用，不命中 command allowlist、也不持久化 always-allow。
## Requirements
### Requirement: 可注入的权限门

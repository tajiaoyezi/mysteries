...
- TO: `### Requirement: stable release 只有在公开产物复核后才可归档`
...
v1.3.0 release notes MUST 如实说明 Agent execution scope与单层只读`delegate_task`的已交付边界，并继续披露没有per-response delegate occurrence硬上限、跨child token总预算、child-only扫描字节硬上限、递归、后台任务、child session及写入/Network child；不得把active child≤4或单child 8+1次Provider调用描述为总token、总输出、总扫描量或总内存的固定上界。本 change未修改config/session wire，不得声称存在新的持久化格式迁移。

#### Scenario: v1.3.0 release notes 固化当前能力
- **WHEN** v1.3.0 candidate固化Changelog与README并生成正式archives
- **THEN** execution scope和单层只读委派从`Unreleased`进入带日期的`1.3.0`历史，新的Changelog `Unreleased`保持空，archive内README把本次能力标记为v1.3.0且限制与兼容边界如实可见

#### Scenario: Implementation merge 不提前产生 404
- **WHEN** v1.3.0 implementation PR已合入但tag/Release尚未公开
- **THEN** README可如实标明源码/候选版本为v1.3.0，但安装命令仍解析到真实公开的v1.2.0 assets或其他实际latest stable，不引用不存在的v1.3.0 URL；v1.3.0公开验证后相同动态命令自动解析到v1.3.0

#### Scenario: 既有发布历史保持真实
- **WHEN** 阅读版本链接与历史release notes
- **THEN** 1.0/1.1仍是无tag/Release的开发里程碑，v1.2.0仍是首个自动化Release，v1.3.0链接到自己的真实tag/Release且不改写旧版本内容
